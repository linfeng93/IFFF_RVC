...
model = dict(
    ...
    neck=dict(
        type='NASFPN',
        stack_times=7,
        in_channels=[746, 1492, 4103, 10444],
        out_channels=256,
        start_level=1,
    ...
    roi_head=dict(
        ...
        bbox_head=[
            dict(
                ...
                loss_cls=dict(
                    type='HierarchyLoss',
                    hierarchy_file="./label_spaces/hierarchy_oid.json",
                    loss_weight=1.5),
...
train_pipeline = [
    ...
    dict(type='Resize',
        img_scale=[
            (1600, 960), (1536, 928), (1472, 896), (1440, 864), (1408, 832),
            (1333, 800), (1280, 768), (1216, 736), (1152, 704), (1120, 672),
            (1088, 640), (1024, 608), (960, 576), (896, 544), (832, 512),
            (800, 480)],
...
oid = dict(
    ...
    dataset=dict(
        type='CocoDataset',
        ann_file='./data/rvc/annotations/oid_boxable.rvc_train.json',
        img_prefix='./data/rvc/annotations/',
        classes=CLASSES,
        pipeline=train_pipeline))
mvd = dict(
    ...
    dataset=dict(
        type='CocoDataset',
        ann_file='./data/rvc/annotations/mvd_boxable.rvc_train.json',
        img_prefix='./data/rvc/annotations/',
        classes=CLASSES,
        pipeline=train_pipeline))
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=8,
    train_dataloader=dict(class_aware_sampler=dict(num_sample_class=1)),  # class-aware sampling
    train=[coco, oid, mvd],
    val=dict(
        type='CocoDataset',
        ann_file= [
            './data/rvc/annotations/coco_boxable.rvc_val.json',
            './data/rvc/annotations/oid_boxable.rvc_val.json',
            './data/rvc/annotations/mvd_boxable.rvc_val.json',
        ],
        img_prefix=['./data/rvc/annotations/', './data/rvc/annotations/', './data/rvc/annotations/'],
        classes=CLASSES,
...
checkpoint_config = dict(interval=1, max_keep_ckpts=10, save_last=True)
