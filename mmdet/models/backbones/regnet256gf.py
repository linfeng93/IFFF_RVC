...
class RegNet256gf(BaseModule):
    ...
    ckpt_path = "checkpoints/mmdet_SEER-regnet256gf.pth"

    def __init__(self):
        super(RegNet256gf, self).__init__()
        model_config = self._model_config
        self.model_scale = model_config["model_scale"]
        self.frozen_stages = model_config["frozen_stages"]
        self.use_checkpointing = model_config["use_checkpointing"]
    ...
    def forward(self, x, out_features=[1, 2, 3, 4]):
        ...
        for i, block in enumerate(self._feature_blocks):  # i = 0 ~ 4
            if self.use_checkpointing:
                x = checkpoint.checkpoint(self._feature_blocks[block], x)
            else:
